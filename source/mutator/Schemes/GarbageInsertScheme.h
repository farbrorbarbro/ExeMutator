...
class GarbageInsertScheme : public MutationScheme
{
public:
	...
	uint32_t	getMaxRandomBlockSize() const;
	void		setMaxRandomBlockSize(uint32_t val);

private:

	uint32_t	generateBlockSize(uint32_t max);
	uint32_t	insertGarbage(InstructionList &instructionList, uint32_t startIndex);

private:

	uint32_t minBlock			= 1;
	uint32_t maxBlock			= 5;
	uint32_t maxRandomBlockSize	= 5;
};